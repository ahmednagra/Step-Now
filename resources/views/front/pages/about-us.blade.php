...
{{-- ============================================================================
     ...
     Wave 2C revisions:
       • De-duplication: this page used to include the SAME about-2 partial
         that's on the homepage, plus the SAME why-choose-us and SAME
         testimonials. Visitors got nothing new. Now this page has its
         OWN content:
            - Owner story / personal intro
            - Operating philosophy ("how we work")
            - Fleet description (placeholder list, ready for real photos)
            - Service-area map mention
            - Numbers / commitments (KPIs, not random vanity stats)
            - Optional: include Why Choose Us if data is present
            - Optional: include Testimonials if data is present
       • Page-header background uses 'about-bg.jpg' (unique to this page)
       • Breadcrumb separator '›'
       • All copy is industry-specific (transport, not workshop)
============================================================================ --}}

@section('title', app()->getLocale() === 'en' ? 'About Us' : 'Über uns')
...
<section class="page-header">
    <div class="page-header__bg" style="background-image: url({{ asset('front/assets/images/about-bg.jpg') }});" aria-hidden="true"></div>
        ...
        <div class="row align-items-center g-4">
            ...
            <div class="col-12 col-lg-5">
                <div style="position:relative;">
                    <img src="{{ asset('front/assets/images/owner-portrait.jpg') }}"
                         alt="{{ $isEN ? 'Naeem Ahmad — owner of StepNow Rides & Movers' : 'Naeem Ahmad — Inhaber StepNow Rides & Movers' }}"
                    ...
                    <div class="position-absolute"
                        ...
                        <strong style="color:var(--sn-ink);font-size:var(--sn-fs-h6);">
                            {{ $isEN ? 'Family-run since 2026' : 'Familienbetrieb seit 2026' }}
                        </strong>
                    </div>
                </div>
            </div>
            ...
            <div class="col-12 col-lg-7">
                <span class="section-title__tagline">{{ $isEN ? 'About the owner' : 'Über den Inhaber' }}</span>
                <h2 class="section-title__title mt-2">
                    {{ $isEN ? 'Local, licensed, and personally accountable.' : 'Regional, konzessioniert und persönlich verantwortlich.' }}
                </h2>
                ...
                <div class="d-flex flex-wrap gap-2 mt-4">
                    <a href="{{ lroute('front.pricing') }}" class="thm-btn">
                        {{ $isEN ? 'See our pricing' : 'Unsere Preise' }}
                        <span class="fas fa-arrow-right" aria-hidden="true"></span>
                    </a>
                    <a href="tel:{{ $phoneE164 }}" class="thm-btn thm-btn--outline">
...
<section>
    <div class="container">
        <div class="text-center mb-5">
            <span class="section-title__tagline">{{ $isEN ? 'Our fleet' : 'Unser Fuhrpark' }}</span>
            <h2 class="section-title__title mt-2">
                {{ $isEN ? 'The right vehicle for every trip' : 'Das passende Fahrzeug für jede Fahrt' }}
            </h2>
                ...
                <div class="col-12 col-md-6 col-lg-4">
                    <article class="sn-card sn-card--interactive h-100">
                        <div class="mb-3" style="width:64px;height:64px;border-radius:var(--sn-radius-lg);background:var(--sn-primary-100);display:flex;align-items:center;justify-content:center;">
                            <i class="fas fa-{{ $vehicle['icon'] }}" style="color:var(--sn-primary);font-size:28px;" aria-hidden="true"></i>
                        </div>
                        ...
                        <ul class="list-unstyled mt-3 mb-0">
                            @foreach ($vehicle['features'] as $feat)
                                <li class="d-flex align-items-start mb-1">
                                    ...
                                    <span style="font-size:var(--sn-fs-sm);color:var(--sn-ink-2);">{{ $feat }}</span>
                                </li>
                            @endforeach
                        </ul>
                    </article>
    ...
    <div class="container">
        <div class="row align-items-center g-4">
            <div class="col-12 col-lg-6">
                <span class="section-title__tagline">{{ __('Service Area') }}</span>
                <h2 class="section-title__title mt-2">
                    {{ $isEN ? 'Where we operate' : 'Wo wir unterwegs sind' }}
                </h2>
                ...
                <div class="d-flex flex-wrap gap-1 mt-4">
                    @foreach (['Deizisau','Esslingen am Neckar','Plochingen','Reichenbach an der Fils','Wernau','Köngen','Wendlingen am Neckar','Altbach','Lichtenwald','Stuttgart','Stuttgart Airport (STR)','Messe Stuttgart'] as $city)
                        <span class="sn-badge sn-badge--primary">{{ $city }}</span>
                    @endforeach
                </div>
            </div>

            <div class="col-12 col-lg-6">
                <a href="{{ lroute('front.contactus') }}" style="display:block;text-decoration:none;">
                    <div class="sn-card sn-card--interactive text-center" style="padding:48px 24px;">
                        ...
                        <h3 style="font-size:var(--sn-fs-h4);">
                            {{ $isEN ? 'See exact pickup point on the map' : 'Abholpunkt auf der Karte ansehen' }}
                        </h3>
                        ...
                        <span class="thm-btn thm-btn--ghost">
                            {{ $isEN ? 'Open contact page' : 'Kontaktseite öffnen' }}
                            <span class="fas fa-arrow-right" aria-hidden="true"></span>
                        </span>
                    </div>
                </a>
...
{{-- ===== Optional admin-managed sections (only if data present) ===== --}}
@if (isset($why_choose_us) && $why_choose_us)
    @includeIf('front.partials.why-choose-us.why-choose-us-1')
@endif

@if (isset($testimonial) && $testimonial && $testimonial->details && $testimonial->details->count())
...
<section class="sn-section-tint">
    <div class="container text-center">
        <h2 class="section-title__title">
            {{ $isEN ? 'Ready when you are' : 'Wir sind bereit, wenn Sie es sind' }}
        </h2>
        <p class="sn-prose mx-auto mt-3">
            {{ $isEN
        ...
        <div class="d-flex flex-wrap gap-2 justify-content-center mt-4">
            ...
            <a href="{{ lroute('front.contactus') }}" class="thm-btn thm-btn--outline">
                {{ $isEN ? 'Contact us' : 'Kontaktieren Sie uns' }}
            </a>
        </div>
    </div>
</section>
