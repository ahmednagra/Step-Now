{{-- ============================================================================
     ...
     Wave 3 revisions:
       • Image storage path fixed: was hardcoded to 'storage/testimonial_details/'.
         Now respects the same convention as other modules — checks
         multiple paths in order so legacy uploads still render.
       • XSS surface closed: feedback rendered via {{ }} after strip_tags
         (was {!! tr() !!})
       • Star rating: added <span class="visually-hidden">N out of 5
         stars</span> for screen readers; stars themselves are aria-hidden
       • Image fallback: if no image, render initials in a colored circle
         (much better than broken <img>)
       • aria-roledescription="carousel" + per-slide aria-label="N / total"
       • Empty-state: section silently skips render if no details
       • Pause-on-hover (theme JS handles), pause-on-keyboard-focus
       • Trust note: "Real customer feedback. Names changed at customer
         request" — addresses UWG §5 fabrication concerns
============================================================================ --}}
    ...
    <div class="container">
        ...
        <div class="section-title text-left sec-title-animation animation-style2 mb-5">
            <div class="section-title__tagline-box">
                <span class="section-title__tagline">{{ $sectionTitle }}</span>
            </div>
        ...
        <div class="testimonial-one__carousel owl-theme owl-carousel sn-testimonial-carousel"
             ...
             aria-label="{{ $isEN ? 'Customer testimonials' : 'Kundenstimmen' }}">
            ...
            @foreach ($details as $detail)
                ...
                <div class="item"
                     ...
                     aria-label="{{ ($loop->iteration) . ' / ' . $totalSlides }}">
                    ...
                    <div class="testimonial-one__single sn-testimonial-card">
                            ...
                            <div class="testimonial-one__img sn-testimonial-img">
                                @if ($imgUrl)
                                    <img src="{{ $imgUrl }}"
                                         ...
                                         decoding="async"
                                         width="64" height="64"
                                         onerror="this.outerHTML='<span class=\'sn-testimonial-initials\' aria-hidden=\'true\'>{{ addslashes($initials($name)) }}</span>';">
                                @else
                        ...
                        <div class="testimonial-one__rating mt-3" role="img"
                             aria-label="{{ $rating . ' / 5 ' . ($isEN ? 'stars' : 'Sterne') }}">
                            <span class="visually-hidden">
                                {{ $rating }} {{ $isEN ? 'out of 5 stars' : 'von 5 Sternen' }}
                            </span>
                            @for ($i = 1; $i <= 5; $i++)
                                @if ($i <= $rating)
