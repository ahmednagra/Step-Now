...
$(document).ready(function () {
    const $dropdown = $("#iconDropdown");
    const $search = $("#iconSearch");
    const $preview = $("#iconPreview");
    const $text = $("#iconSelectedText");
    ...
    $search.on("input", function () {
        ...
        const filtered = bootstrapIcons.filter((i) => i.includes(q));
    ...
    function renderIcons(icons) {
        $dropdown.empty();
        icons.forEach((name) => {
            const fullClass = `bi bi-${name}`;
            $dropdown.append(`
          <button type="button" class="dropdown-item d-flex align-items-center icon-option" data-icon="${name}">
            <i class="${fullClass} me-2"></i> ${fullClass}
          </button>
        `);
        });
    }
    ...
    $(document).on("click", ".icon-option", function () {
        ...
        const fullClass = `bi bi-${name}`;
        $preview.attr("class", `${fullClass} fs-4`);
        $text.text(fullClass);
        $value.val("bi bi-" + name); // save only short name to DB
        $dropdown.removeClass("show");
        $search.val("");
    });

    $search.on("focus", () => $dropdown.addClass("show"));

    $(document).on("click", (e) => {
        if (!$(e.target).closest("#iconDropdown, #iconSearch").length) {
            $dropdown.removeClass("show");
        }
    });
});
